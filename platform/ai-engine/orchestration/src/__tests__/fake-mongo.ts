 ...
 * Pattern Recognition, and this package's own ReportStore touch —
 * insertOne/insertMany, findOne, find().sort().skip().limit().toArray(),
 * countDocuments, createIndex, updateOne (with $set/$setOnInsert and
 * upsert), deleteMany.
 *
 * Copied verbatim from @adaptive-ai/memory-engine's own
 * src/__tests__/fake-mongo.ts (same rationale: no reliable network access to
 * a real `mongod` binary in this sandbox) so this package's integration
 * tests can drive Memory Engine + Pattern Recognition + ReportStore against
 * one shared, in-process `Db`-shaped fake exactly the way the real
 * `OrchestrationStack` wires them against one real `Db`.
 */
