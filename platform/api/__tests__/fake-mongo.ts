 ...
 * upsert), deleteMany.
 *
 * Copied verbatim from @adaptive-ai/orchestration's own
 * src/__tests__/fake-mongo.ts (same rationale: no reliable network access to
 * a real `mongod` binary in this sandbox, and the established per-package
 * duplication convention elsewhere in this monorepo) — lets
 * matchStart.test.ts build a real OrchestrationStack (Milestone 1c) without
